...
func main() {
	...
	mux.HandleFunc("/cart", func(w http.ResponseWriter, r *http.Request) {
		...
		url := "http://localhost:6013/api/v1" + r.URL.Path + "?" + r.URL.RawQuery
		if len(token) > 1 {
	...
	mux.HandleFunc("/tracking", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Access-Control-Allow-Headers", "Content-Type,access-control-allow-origin, access-control-allow-headers,Authorization")
		token := strings.Split(r.Header.Get("Authorization"), " ")
		url := "http://localhost:6019/api/v1" + r.URL.Path + "?" + r.URL.RawQuery
		if len(token) > 1 {
			sendReq(token[1], url, w, r)
		} else {
			sendReq(" ", url, w, r)
	...
	mux.HandleFunc("/order", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Access-Control-Allow-Headers", "Content-Type,access-control-allow-origin, access-control-allow-headers,Authorization")
		token := strings.Split(r.Header.Get("Authorization"), " ")
		url := "http://localhost:6016/api/v1" + r.URL.Path + "?" + r.URL.RawQuery
		if len(token) > 1 {
			sendReq(token[1], url, w, r)
		} else {
			sendReq(" ", url, w, r)
	...
	mux.HandleFunc("/payments", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Access-Control-Allow-Headers", "Content-Type,access-control-allow-origin, access-control-allow-headers,Authorization")
		token := strings.Split(r.Header.Get("Authorization"), " ")
		url := "http://localhost:6017/api/v1" + r.URL.Path + "?" + r.URL.RawQuery
		if len(token) > 1 {
			sendReq(token[1], url, w, r)
		} else {
			sendReq(" ", url, w, r)
	...
	log.Fatal(http.ListenAndServe(":6003", handler))
}
