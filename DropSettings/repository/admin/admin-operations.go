...
func NewAdminSettingRepository(collectionName string) AdminSettingRepository {
	return &repo{
...
func (r *repo) UpdateOne(filter, update bson.M) (string, error) {
	...
	if result.MatchedCount == 0 || result.ModifiedCount == 0 {
		err = errors.New("setting not found(404)")
		trestCommon.ECLog3(
			"update setting",
			err,
			logrus.Fields{
...
func (r *repo) FindOne(filter, projection bson.M) (entity.SettingDB, error) {
	var setting entity.SettingDB
	err := trestCommon.FindOne(filter, projection, r.CollectionName).Decode(&setting)
	if err != nil {
		trestCommon.ECLog3(
			"Find setting",
			err,
...
func (r *repo) Find(filter, projection bson.M, limit, skip int) ([]entity.SettingDB, error) {
	var settings []entity.SettingDB
