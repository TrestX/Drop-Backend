...
import (
	controller "Drop/DropItemCategories/controller/category"
...
var (
	categoryService = controller.NewCategoryService(admin.NewAdminRepository("itemcategory"))
)
...
func GetActiveCategory(w http.ResponseWriter, r *http.Request) {
	...
	shopType := ""
	shopTypeS := r.URL.Query().Get("shoptype")
	name := ""
	nameS := r.URL.Query().Get("categoryname")
	if limitS != "" {
	...
	data, err := categoryService.GetActiveCategorys(shopType, name, limit, skip)
	if err != nil {
...
func DeleteCategory(w http.ResponseWriter, r *http.Request) {
	...
	data, err := categoryService.DeleteCategory(categoryID)
	if err != nil {
		trestCommon.ECLog1(errors.Wrapf(err, "unable to update category location"))
...
func GetAllTags(w http.ResponseWriter, r *http.Request) {
	...
	data, err := categoryService.GetAllTags(limit, skip, sType)
	if err != nil {
		trestCommon.ECLog1(errors.Wrapf(err, "unable to get all categories"))
...
func DeleteTag(w http.ResponseWriter, r *http.Request) {
	...
	var categoryID = mux.Vars(r)["tagId"]
	if categoryID == "" {
		trestCommon.ECLog1(errors.Wrapf(err, "unable to update category location"))
		w.WriteHeader(http.StatusNotFound)
		json.NewEncoder(w).Encode(bson.M{"status": false, "error": "Unable to update category location"})
	...
	data, err := categoryService.DeleteTag(categoryID)
	if err != nil {
		trestCommon.ECLog1(errors.Wrapf(err, "unable to update category location"))
